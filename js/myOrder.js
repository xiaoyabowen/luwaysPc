    ...
    function handle(data, param) {
		...
		if (data.data ==  ''){
			...
			var orderData = '<tr>\n' +
                ...
                '\t\t\t\t\t\t</label>\n' +*/
                '\t\t\t\t\t\t<span class="myOrderDel myOrderDetele">删除</span>\n' +
                '\t\t\t\t\t</th>\n' +
			...
			for (var i=0;i<data.data.length;i++){
                ...
                orderDataMain += '<tr id="'+data.data[i].id+'">\n'
                ...
                orderDataMain +='\t\t\t\t\t<td style=" text-align: left;" class="myOrderClassInput">\n' +
                    '\t\t\t\t\t\t\t<input type="radio" class="myOrderHaveCheck"  id="model'+i+'"  data-shopId="'+data.data[i].id+'" name="checkItem" /> \n' +
					...
					if(data.data[i].orderType == '0'){
                        orderDataMain += '<td class="fontType">当地玩乐</td>'
					}else if (data.data[i].orderType == '1'){
                        orderDataMain += '<td class="fontType">酒店</td>'
					}else if (data.data[i].orderType == '2'){
                        orderDataMain += '<td class="fontType">发送voucher</td>'
                    }else if (data.data[i].orderType == '3'){
                        orderDataMain += '<td class="fontType">voucher直连下单</td>'
                    }else{
                        orderDataMain += '<td class="fontType">未知状态</td>'
                    }
					...
					if(data.data[i].orderStatus == '0'){
                        ...
                        orderDataMain +='确认成功'
                    }else if (data.data[i].orderStatus == '15'){
                        orderDataMain +='待确认'
                    }else if (data.data[i].orderStatus == '16'){
                        orderDataMain +='确认中'
...
// 删除按钮
$(document).on("click",".myOrderDetele",function(){
    ...
    if ($("input[name='checkItem']:checked").val() == null) {
        ...
        $(".dialogue-sure").click(function(){
            myAjax(orderRestControllerDeletepro, {orderId:shopId}, handle)
            function handle(data, param) {
                console.log(data)
...
$(document).on("click",".myOrderDetail",function () {
    ...
    localStorage.setItem("allData",allDataDetailJson)
    if ($(".fontType").html() == "酒店"){
        window.location.href = "../html/hotelOrderPayGoDetail.html"
    }else if ($(".fontType").html() == "当地玩乐"){
